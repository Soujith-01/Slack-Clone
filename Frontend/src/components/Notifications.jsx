...
function Notifications() {
  ...
  const fetchRequests = async () => {
    try {
      const response = await axios.get(`${BACKEND}/chat-api/chats/channels`, {
      ...
      const filtered = channels.filter(
        (channel) => channel.joinRequests && channel.joinRequests.length > 0
      );
  ...
  // APPROVE / REJECT
  const handleRequest = async (channelId, request, approve) => {
    // ✅ handle both populated { _id } and plain ObjectId string
  ...
  const totalRequests = requests.reduce(
    (acc, channel) => acc + channel.joinRequests.length,
    0
  );
      ...
      {open && (
        <div className="absolute right-0 mt-4 w-[380px] bg-[#171A22] border border-[#2A2F3A] rounded-[24px] shadow-2xl overflow-hidden z-50">
          ...
          <div className="px-5 py-4 border-b border-[#2A2F3A]">
            ...
            <p className="text-sm text-[#8B94A7] mt-1">Channel join requests</p>
          </div>
          ...
          <div className="max-h-[450px] overflow-y-auto">
            ...
            {totalRequests === 0 ? (
              ...
              requests.map((channel) => (
                <div key={channel._id} className="border-b border-[#232734]">

                  {channel.joinRequests.map((request) => {
                    // ✅ stable key that works whether user is populated or not
                    const userId = request.user?._id || request.user;
                    const key = `${channel._id}-${userId}`;
                        ...
                        <div className="flex gap-4">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex gap-2 mt-4">
                              ...
                              <button
                                disabled={loading}
                                onClick={() => handleRequest(channel._id, request, true)}
                              ...
                              <button
                                disabled={loading}
                                onClick={() => handleRequest(channel._id, request, false)}
